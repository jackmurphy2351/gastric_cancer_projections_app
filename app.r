...
source("gastric_mortality_calculator.R")
...
server <- function(input, output, session) {
    ...
    withProgress(message = 'Calculating mortality reductions...', value = 0, {
      ...
      tryCatch({
        calc_results <- calculate_gastric_mortality_reduction(
          data_file = "~/gastric_cancer_projections_app/data/gastric_cancer_parameters.csv",
  ...
  output$total_groups_box <- renderInfoBox({
    if (!results$available) {
      infoBox("Groups Analyzed", "—", icon = icon("users"), color = "blue")
    } else {
      infoBox("Groups Analyzed", nrow(results$data), icon = icon("users"), color = "blue")
    }
  })
