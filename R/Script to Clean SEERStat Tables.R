...
setwd("C:/Users/murphyjd/OneDrive - National Institutes of Health/IM_modelling study")
...
# Read in data
census_17_raw <- read_csv(file = "Raw Census 2017 Projections.csv", col_names = T, trim_ws = T, col_types = "n")
...
calculate_and_check_sum <- function(row_number, data_clean, data_clean2) {
  ...
  for (start_col in seq(1, ncol(data_clean), by = 5)) {
    ...
    sum_cols <- sum(data_clean[row_number, start_col:end_col])
...
raw_inc_dat <- read_csv(file = "NordPred Incidence by Subsite and Histological Subtype Matrix 20231109.csv", col_names = T, na = "^", trim_ws = T, )
...
## Read in data
ibmort_apcs_dat <- read_xlsx("Gastric Cancer Incidence-Based Mortality APCs 20231128.xlsx", col_names = T, na = "~")
...
raw_ibmort_dat <- read_csv(file = "Gastric Cancer Incidence-Based Mortality 20231128.csv", col_names = T, na = "^", trim_ws = T)
...
raw_mort_dat <- read_csv(file = "IM Modelling Mortality by Race-Ethnicity and Age 20231025.csv", col_names = T, na = "^", trim_ws = T)
